...
void selectionMenu(int *valMenu){
  int val = 0;
  do {
    printf(""ROSE"Sélectionner une Opération ?"Class"\n");
    printf(">");
    scanf("%d",&val);
...
void selectFich(selection *select){
  ...
  //selection
  do {
    printf(""VERT"Le nom sélectioner ?"Class"\n");
    printf(">");
    scanf("%s", trans);
    snprintf(cmdtrans, sizeof cmdtrans, "cd ./data/%s/", trans);
  } while(system(cmdtrans) != 0);
...
void executionSelect(int valMenu, BDD *select, int *etatSelect){
  ...
  switch (valMenu) {
    ...
    //on vérifie si l'ultilisateur à sélectionné un directory
      if(*etatSelect == 0){
        printf(""ROUGE"IMPOSSIBLE MALHEUREUX SELECTIONNE UNE BASE DE DONNEES"Class"\n");
      }
      else{
        gerePoint(select);
      }
